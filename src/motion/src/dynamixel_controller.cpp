...
#include "motion/dynamixel_controller.h"
     ...
     priv_node_handle_("~")
{
  ...
  joint_id_.push_back(priv_node_handle_.param<int>("joint9_id", 9));
  joint_id_.push_back(priv_node_handle_.param<int>("joint11_id", 11));
  joint_id_.push_back(priv_node_handle_.param<int>("joint13_id", 13));
  joint_id_.push_back(priv_node_handle_.param<int>("joint15_id", 15));
  ...
  ROS_INFO("dynamixel_controller [node] : Init OK!");
}
...
void DynamixelController::updateJointStates()
{
  ...
  float joint_states_pos[JOINT_NUM] = {0.0, };
  float joint_states_vel[JOINT_NUM] = {0.0, };
  float joint_states_eff[JOINT_NUM] = {0.0, };

  double get_joint_position[JOINT_NUM] = {0.0, };
  ...
  joint_state.name.push_back("joint11");
  joint_state.name.push_back("joint13");
  joint_state.name.push_back("joint15");
...
void DynamixelController::goalJointPositionCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
  ...
  ROS_INFO("[%d][%d][%d][%d]",goal_position[0],goal_position[1],goal_position[2],goal_position[3]);
